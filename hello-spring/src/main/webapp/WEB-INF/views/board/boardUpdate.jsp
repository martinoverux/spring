				...
				<c:forEach items="${board.attachments}" var="attach" varStatus="vs">
					<div class="btn-group-toggle p-0 mb-3" data-toggle="buttons">
	                	<label class="btn btn-outline-danger btn-block" title="${attach.originalFilename} 삭제">
	                    	<input type="checkbox" id="delFile${vs.count}" name="delFile" value="${attach.no}" checked> 
...
const delLength = document.querySelectorAll("[name=delFile]").length;
for(let i = 0; i < delLength; i++){
	 document.querySelectorAll("input[type=file]")[i].parentElement.parentElement.style.display = "none";
}
...
document.querySelectorAll("[name=delFile]").forEach((delFile) => {
	delFile.onchange = (e) => {
		...
		const file = document.querySelector(`#upFile\${n}`).parentElement.parentElement;
		file.style.display = checked ? "flex" : "none";
		checked || (file.value = "");
		del.style.display = checked ? "none" : "flex";
		console.log(e.target.checked);
	}
});
